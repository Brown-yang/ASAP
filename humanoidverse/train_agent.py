...
def main(config: OmegaConf):
    # print(OmegaConf.to_yaml(config))  # 打印完整的配置，查看是否正确传入
    ...
    print("aaaaaaaaaaaaaaaawwwwwww")
    algo.setup()
    # import ipdb;    ipdb.set_trace()
    if config.checkpoint is not None:
        algo.load(config.checkpoint)
    logger.info(f"查看config配置{config.checkpoint}")

    print(algo.actor_critic)
    print("888888888888")
    # handle saving config
    algo.learn()        #这里就是模型进行学习的地方，模型训练#######  这里使用的模型就是ppo
