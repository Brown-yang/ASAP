...
from humanoidverse.agents.modules.actor_ctritic import ActorCritic

console = Console()

# PPO是一个强化学习的算法，其基类为BaseAlgo,这里根本没有定义ActorCritic  对该代码进行修改
class PPO(BaseAlgo):
    def __init__(self,
                 env: BaseTask,
                 config,
        ...
        # PPO components
        self.actor_critic = ActorCritic(obs_dim_dict_actor=self.algo_obs_dim_dict,
                                        module_config_dict_actor=self.config.module_dict.actor,
                                        obs_dim_dict_critic=self.algo_obs_dim_dict,
                                        module_config_dict_critic=self.config.module_dict.critic,
    ...
    def _setup_models_and_optimizer(self):
        #####这里的actor和critic都指向一个对象，分别去更新actor和critic的参数
        self.actor = self.actor_critic.to(self.device)
        self.critic = self.actor_critic.to(self.device)
    ...
    def load(self, ckpt_path):
        ...
        if ckpt_path is not None:
            ...
            #################这两行代码相当于加载模型的权重，也就是加载actor和critor的权重信息
    ...
    def _rollout_step(self, obs_dict):
        with torch.inference_mode():
            for i in range(self.num_steps_per_env):
                ...
                policy_state_dict = {}
                policy_state_dict = self._actor_rollout_step(obs_dict, policy_state_dict)      ######这里出错
